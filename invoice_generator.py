...
class InvoiceGenerator:
    def __init__(
        ...
        self.invoice_due_date: datetime = self.invoice_date + timedelta(
            days=invoice_due_period
        )
        ...
        self.layout = SingleColumnLayout(
            self.page,
            vertical_margin=(self.page.get_page_info().get_height() * Decimal(0.02)),
        )
    ...
    def build_billing_shipping(self):
        ...
        info.add(
            Paragraph(
                "BILL TO",
                background_color=TOP_ROW_TABLE_COLOR,
        ...
        # Rest of the adress
        info.add(
            Paragraph(f"{self.client.billing_info.city_province_areacode}")
        )  # BILLING
        info.add(
            Paragraph(f"{self.client.shipping_info.city_province_areacode}")
        )  # SHIPPING
    ...
    def build_items(self, items_to_bill: List[BillableItem]):
        ...
        for heading in ["DESCRIPTION", "QTY", "UNIT PRICE", "AMOUNT"]:
            item_table.add(
                TableCell(
                    Paragraph(heading, font_color=X11Color("White")),
                    background_color=TOP_ROW_TABLE_COLOR,
                )
            )
        ...
        for i in range(0, 10):
            ...
            if i < len(items_to_bill):
                ...
                item_table.add(
                    TableCell(
                        Paragraph(items_to_bill[i].item_description),
                ...
                item_table.add(
                    TableCell(
                        Paragraph(str(items_to_bill[i].item_quantity)),
                        background_color=row_color,
                    )
                )
                # Unit Price
                item_table.add(
                    TableCell(
                        Paragraph(f"${items_to_bill[i].item_unit_price:.2f}"),
                        background_color=row_color,
                    )
                )
                ...
                # Total Amount
                item_amount = (
                    items_to_bill[i].item_quantity * items_to_bill[i].item_unit_price
                )
                sub_total += item_amount
                item_table.add(
                    TableCell(Paragraph(f"${item_amount}"), background_color=row_color)
                )
            else:
                # No more items, fill in the rest of the rows
                for _ in range(0, 4):
                    item_table.add(
                        TableCell(Paragraph(" "), background_color=row_color)
                    )
        ...
        item_table.add(
            TableCell(
                Paragraph("Sub-Total", horizontal_alignment=Alignment.RIGHT), col_span=3
            )
        )
        item_table.add(
            TableCell(
                Paragraph(f"${sub_total:.2f}", horizontal_alignment=Alignment.RIGHT),
                col_span=1,
            )
        )
        ...
        item_table.add(
            TableCell(
                Paragraph(
                    f"GST ({self.tax_rate}%)", horizontal_alignment=Alignment.RIGHT
                ),
                col_span=3,
            )
        )
        item_table.add(
            TableCell(
                Paragraph(f"${(tax_amount):.2f}", horizontal_alignment=Alignment.RIGHT),
                col_span=1,
            )
        )
        ...
        item_table.add(
            TableCell(
                Paragraph("Total", horizontal_alignment=Alignment.RIGHT), col_span=3
            )
        )
        item_table.add(
            TableCell(
                Paragraph(
                    f"${ (tax_amount + sub_total):.2f}",
                    horizontal_alignment=Alignment.RIGHT,
                ),
                col_span=1,
            )
        ...
        item_table.set_padding_on_all_cells(
            Decimal(2), Decimal(2), Decimal(4), Decimal(2)
        )
...
if __name__ == "__main__":
    ...
    test_items = [
        BillableItem("Engineering Hours", 40, 60),
        BillableItem("Consulting fees", 12, 120),
    ]
    ...
    # TODO: Add GST number to the form
